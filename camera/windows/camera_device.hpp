...
public:
    ...
    IFACEMETHODIMP OnEvent(IMFMediaEvent* media_event) override {
        bool camera_stopped = false;
        {
            ...
            if (IsEqualGUID(capture_event_guid, MF_CAPTURE_ENGINE_INITIALIZED)) {
                observer_->SetCameraStopped(false);
            } else if (IsEqualGUID(capture_event_guid, MF_CAPTURE_ENGINE_PREVIEW_STOPPED)) {
                camera_stopped = true;
            }

        }
        // call only once if the camera is stopped
        if (camera_stopped) {
            observer_->SetCameraStopped(true);
        }
        return S_OK;
    }
    ...
    IFACEMETHODIMP OnSample(IMFSample* sample) override {
        ...
        */
        ...
        for (DWORD i = 0; i < count; ++i) {
            ...
            if (!observer_->IsCameraStopped() && buffer) {
...
public:
    ...
    void SetCameraStopped(bool val) {
        m_camera_stopped = val;
    }

    bool IsCameraStopped() {
        return m_camera_stopped;
    }

    void Stop() {
        HRESULT hr = engine_->StopPreview();
        if (FAILED(hr)) { assert(0); }
        // waiting for the camera to stop
        while (!m_camera_stopped) {
...
private:
    ...
    std::atomic_bool m_camera_stopped = true;
};
